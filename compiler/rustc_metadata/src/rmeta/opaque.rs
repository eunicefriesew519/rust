...
pub(super) type FileEncodeResult = Result<usize, (PathBuf, io::Error)>;

pub(super) const MAGIC_END_BYTES: &[u8] = b"rust-end-file";
...
pub(super) struct FileEncoder<'a> {
    ...
    res: Result<(), io::Error>,
    path: PathBuf,
    stable_hasher: &'a mut StableHasher,
    _hcx: StableHashingContext<'a>,
...
impl<'a> FileEncoder<'a> {
    ...
    pub(super) fn flush(&mut self) {
        ...
        self.stable_hasher.write(&self.buf[..self.buffered]);
        self.buffered = 0;
    }
    ...
    #[inline]
    pub(super) fn write_array<const N: usize>(&mut self, buf: [u8; N]) {
    ...
    pub(super) fn finish(&mut self) -> FileEncodeResult {
    ...
    pub(super) fn hash(&mut self) -> Fingerprint {
        self.flush();
        self.stable_hasher.clone().finish()
    }
}

#[cfg(debug_assertions)]
impl<'a> Drop for FileEncoder<'a> {
    fn drop(&mut self) {
...
impl<'a> MemDecoder<'a> {
    /*#[inline]
    pub(super) fn new(data: &'a [u8], position: usize) -> Result<MemDecoder<'a>, ()> {
    ...
    #[inline]
    pub(super) fn split_at(&self, position: usize) -> MemDecoder<'a> {
    ...
    #[inline]
    pub(super) fn read_array<const N: usize>(&mut self) -> [u8; N] {
    ...
    #[inline]
    pub(super) fn with_position<F, T>(&mut self, pos: usize, func: F) -> T
    where
...
pub(super) struct IntEncodedWithFixedSize(pub(super) u64);

impl IntEncodedWithFixedSize {
    pub(super) const ENCODED_SIZE: usize = 8;
}
